  ...
  components: {
    ...
    schemas: {
      ...
      DevNombaWebhookSimulateRequest: {
        type: "object",
        description:
          "Simulates a Nomba payment_success/payment_failed event and runs Recurr's real webhook processor. Provide merchantTxRef to settle a payment attempt, or orderReference to activate a pending payment method setup checkout.",
        properties: {
          merchantTxRef: {
            type: "string",
          ...
          orderReference: {
            ...
            description:
              "Must match paymentMethod.providerSetupReference, or metadata.requestedSetupReference, for payment-method setup testing.",
          },
          amountMinor: {
            type: "integer",
            default: 100,
            example: 100,
            description:
              "Amount in kobo/minor units. For payment methods this can be the setup checkout amount; for payment attempts it must match PaymentAttempt.amountMinor.",
          },
          currency: { $ref: "#/components/schemas/Currency" },
          eventType: {
            type: "string",
          ...
          nombaCustomerId: {
            ...
            example: "cus_8821",
            description:
              "Provider customer reference to store on the payment method. If omitted, the simulator generates one.",
          },
          cardBrand: {
            type: "string",
            example: "visa",
      ...
      DevRunDueBillingRequest: {
        ...
        properties: {
          businessId: {
            ...
            format: "uuid",
            description:
              "Business whose due subscriptions should be processed. The merchant session user must belong to this business.",
      ...
      DevFastForwardSubscriptionBillingRequest: {
        ...
        properties: {
          ...
          minutesAgo: {
            ...
            example: 1,
            description:
              "Sets nextBillingAt to now minus this many minutes. Use 0 to set it to now.",
          },
        },
      },
      ...
      DevRunCleanupRequest: {
        ...
        properties: {
          businessId: {
            ...
            format: "uuid",
            description:
              "Optional. Limit cleanup to one business. If provided, the merchant session user must belong to this business.",
            example: "ce640d6a-c392-488c-b81b-c66ebdf42258",
          },
          mode: { $ref: "#/components/schemas/ApiKeyMode" },
  ...
  paths: {
    ...
    "/api/v1/dev/webhooks/nomba/simulate": {
      post: {
        ...
        description:
          "Testing endpoint protected by merchantSession. Creates a Nomba-shaped webhook payload, signs it with NOMBA_WEBHOOK_SECRET/NOMBA_WEBHOOK_SIGNING_KEY, stores it, and runs the same webhook processor used by the real /api/v1/webhooks/nomba endpoint.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
    ...
    "/api/v1/dev/billing/run-due": {
      post: {
        ...
        description:
          "Protected demo/testing endpoint. Finds due ACTIVE/TRIALING subscriptions for one business, creates renewal invoice/payment attempt, and charges the saved payment method.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
    ...
    "/api/v1/dev/billing/subscriptions/{id}/fast-forward": {
      post: {
        ...
        description:
          "Protected demo/testing endpoint. Sets an ACTIVE/TRIALING subscription's nextBillingAt to now or a few minutes in the past so the billing worker can renew it immediately.",
        parameters: [
          {
            name: "id",
            in: "path",
    ...
    "/api/v1/dev/cleanup/run": {
      post: {
        ...
        description:
          "Protected testing endpoint. Expires old portal sessions, fails stale PAYMENT_PROCESSING invoices, cancels stale INCOMPLETE subscriptions, and deletes old idempotency keys.",
        requestBody: {
          required: false,
          content: {
            "application/json": {
    ...
    "/api/v1/businesses/{businessId}/webhook-endpoints": {
      ...
      post: {
        ...
        description:
          "Creates an endpoint that receives signed Recurr lifecycle events. The signingSecret is returned once and should be stored by the merchant.",
        parameters: [
          { name: "businessId", in: "path", required: true, schema: { type: "string", format: "uuid" } },
        ],
        requestBody: {
    ...
    "/api/v1/businesses/{businessId}/webhook-endpoints/{id}": {
      ...
      delete: {
        ...
        description:
          "Soft lifecycle action. Existing delivery history is retained.",
        parameters: [
          { name: "businessId", in: "path", required: true, schema: { type: "string", format: "uuid" } },
          { name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } },
        ],
    ...
    "/api/v1/businesses/{businessId}/webhook-endpoints/{id}/test": {
      post: {
        ...
        description:
          "Sends webhook_endpoint.test to the endpoint immediately and stores the delivery result.",
        parameters: [
          { name: "businessId", in: "path", required: true, schema: { type: "string", format: "uuid" } },
          { name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } },
        ],
    ...
    "/api/v1/portal/sessions": {
      post: {
        ...
        description:
          "Creates a short-lived hosted billing portal session for one active customer under the API key business/mode. The returned token is only shown once.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
        ...
        responses: {
          "201": {
            description:
              "Portal session created. Returns url and raw token once.",
          },
        },
      },
      get: {
    ...
    "/api/v1/portal/sessions/{token}": {
      get: {
        ...
        description:
          "Public subscriber-facing endpoint. Validates the opaque portal token and returns customer billing context: subscriptions, invoices, and masked payment methods.",
        parameters: [
