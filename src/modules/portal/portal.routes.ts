...
import {
  portalCancelSubscriptionSchema,
  portalChangePlanSchema,
  portalInvoicePayParamsSchema,
  portalInvoicePaySchema,
  portalPaymentMethodSetupSchema,
  createPortalSessionSchema,
  listPortalSessionsQuerySchema,
  portalSubscriptionActionParamsSchema,
...
async function requireActivePortalSession(token: string) {
...
function successfulProviderStatus(status: string) {
  return /success|successful|succeeded|paid|approved/i.test(status);
}

function compatibleForImmediateProration(input: {
  oldPlan: { currency: string; interval: string; intervalCount: number };
  newPlan: { currency: string; interval: string; intervalCount: number };
}) {
  return (
...
portalRouter.post(
  "/sessions",
  businessResourceAuthMiddleware,
...
portalRouter.post(
  "/sessions/:token/invoices/:invoiceId/pay",
  validate({
    params: portalInvoicePayParamsSchema,
    body: portalInvoicePaySchema,
  }),
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      throw new ApiError(
        ...
        [{ failureReason, paymentAttemptId: updatedAttempt.id, invoiceId: invoice.id }],
...
portalRouter.post(
  "/sessions/:token/payment-methods/setup-checkout",
  validate({
    params: portalSessionTokenParamsSchema,
    body: portalPaymentMethodSetupSchema,
  }),
  idempotencyMiddleware,
  asyncHandler(async (req, res) => {
    const portalSession = await requireActivePortalSession(String(req.params.token));
...
portalRouter.post(
  "/sessions/:token/subscriptions/:subscriptionId/cancel",
  validate({
    params: portalSubscriptionActionParamsSchema,
    body: portalCancelSubscriptionSchema,
  }),
  asyncHandler(async (req, res) => {
    const portalSession = await requireActivePortalSession(String(req.params.token));
    ...
    const subscription = req.body.cancelAtPeriodEnd
      ...
      : await prisma.subscription.update({
          where: { id: existing.id },
          data: subscriptionTransitionData(existing.status, "CANCELLED"),
        });
...
portalRouter.post(
  "/sessions/:token/subscriptions/:subscriptionId/change-plan",
  validate({
    params: portalSubscriptionActionParamsSchema,
    body: portalChangePlanSchema,
  }),
  idempotencyMiddleware,
  asyncHandler(async (req, res) => {
    const portalSession = await requireActivePortalSession(String(req.params.token));
    ...
    if (charge.status === "SUCCEEDED") {
      ...
      if (successfulProviderStatus(verification.status)) {
        ...
        sendSuccess(res, 200, "Subscription plan changed", {
          ...result,
          action: "CHANGED",
          subscription,
          invoice,
    ...
    if (charge.status === "FAILED") {
      ...
      sendSuccess(res, 200, "Subscription plan change payment failed", {
        ...result,
    ...
    sendSuccess(res, 200, "Subscription plan change payment is processing", {
      ...result,
      paymentAttempt,
      paymentProviderResult: sanitizeProviderResult(charge),
    });
