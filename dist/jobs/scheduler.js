...
function intervalMinutesMs(envName, fallbackMinutes) {
    const value = Number(process.env[envName]);
...
function scheduleRecurringJobs(input) {
    ...
    const cleanupInterval = intervalMs("WORKER_CLEANUP_INTERVAL_MS", intervalMinutesMs("CLEANUP_REPEAT_MINUTES", 15));
