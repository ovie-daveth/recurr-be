...
async function deliverToEndpoint(input) {
    ...
    try {
        ...
        if (!delivered) {
            (0, observability_1.incrementMetric)("webhooks.delivery_failed", {
                businessId: input.endpoint.businessId,
                eventType: input.payload.type,
                final: !nextAttemptAt,
            });
            (0, observability_1.observeEvent)("warn", "webhooks.delivery_failed", {
                businessId: input.endpoint.businessId,
                endpointId: input.endpoint.id,
        ...
        const failureReason = error instanceof Error ? error.message : "Webhook delivery failed";
        (0, observability_1.incrementMetric)("webhooks.delivery_failed", {
            businessId: input.endpoint.businessId,
            eventType: input.payload.type,
            final: !nextAttemptAt,
        });
        (0, observability_1.observeEvent)("error", "webhooks.delivery_failed", {
            businessId: input.endpoint.businessId,
            endpointId: input.endpoint.id,
            deliveryId: delivery.id,
            eventId: input.payload.id,
